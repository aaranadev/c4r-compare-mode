...
const platform = PLATFORMS.CARTO_CLOUD_NATIVE

async function promptArgs({ prompter, args, questions }) {
  const answers = await prompter.prompt(
    questions.filter(({ name }) => !args[name]),
  )
  return { ...answers, ...args }
}
  ...
  const processedFiles = files
    ...
    .reduce((total, file) => {
      ...
      if (typeof file === 'string') {
        processedFile = {
          path: file.split(`src${path.sep}`)[
            file.split(`src${path.sep}`).length - 1
          ],
      ...
      if (processedFile.name.includes('.ts')) {
