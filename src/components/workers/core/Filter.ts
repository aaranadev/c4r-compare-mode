...
import { FiltersLogicalOperators } from './constants/FiltersLogicalOperators'
...
function passesFilter(
  ...
  return columns[method]((column: any) => {
...
export function buildBinaryFeatureFilter({ filters = {} }) {
  ...
  return (featureIdIdx: any, binaryData: any) =>
...
function getBinaryPropertyByFilterValues(filterValues: any) {
  return typeof filterValues.flat()[0] === 'string'
    ? 'properties'
    : 'numericProps'
}

function getFeatureValue(featureIdIdx: any, binaryData: any, filter: any) {
...
function passesFilterUsingBinary(
  ...
  return columns.every((column: any) => {
    const columnFilters = filters[column]

    return Object.entries(columnFilters).every(([type, { values }]: any) => {
