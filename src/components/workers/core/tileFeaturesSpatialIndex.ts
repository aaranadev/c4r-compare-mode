...
import { SpatialIndex } from './constants/SpatialIndexTypes'
...
export default function tileFeaturesSpatialIndex({
  ...
  // @ts-ignore
  const cellsDictionary = cells.reduce((prev: any, current: any) => {
    prev[current] = {}
    return prev
  }, {})
...
function getResolution(tiles: any, spatialIndex: any) {
  const data = tiles.find((tile: any) => tile.data?.length)?.data
