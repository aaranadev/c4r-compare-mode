...
export function applySorting(
  features: any,
  { sortBy, sortByDirection = 'asc' }: any = {},
) {
...
// Aux
function createSortFn({ sortBy, sortByDirection }: any) {
...
function normalizeSortByOptions({ sortBy, sortByDirection }: any) {
  if (!Array.isArray(sortBy)) {
    sortBy = [sortBy]
  }

  return sortBy.map((sortByEl: any) => {
    // sortByEl is 'column'
